	...
	factory: (require) => {
		...
		*/
		...
		const zh = {
			...
			shelf: "记忆写入的 shelf",
			shelfHint: "插件的记录、整合与会话开始时的规则预加载都读写这个 shelf；不是 default 时，会话开头会提示模型为 hypatia 命令加上 --shelf。保存后采集器会自动重启，改写到新的 shelf。每个 shelf 的记录进度单独保存，切回原来的 shelf 会从上次的位置继续；切到新的 shelf 后，每个有活动的会话会把完整历史重新记录进去并整合一遍（会消耗模型调用）。列表来自 hypatia list，打开本页时按需读取。",
			...
			advancedHint: "高级字段（二进制、收集器上限、队列调优）可在「设置 → 插件」的本插件页，或 profile patch 的 {ns} 条目 config 下编辑。",
			reset: "重置",
			discard: "放弃",
			save: "保存",
			saving: "保存中…",
		...
		const en = {
			...
			advancedHint: "Advanced fields (binaries, collector caps, queue tuning) can be edited in Settings → Plugins under this plugin, or under the {ns} entry's config in the profile patch.",
			reset: "Reset",
			discard: "Discard",
			save: "Save",
			saving: "Saving…",
		...
		*/
		...
		function getSnapshotValue(form) {
			return form.getSnapshot();
		}
		function subscribe(form, cb) {
			return form.subscribe(cb);
		}
		function sectionValue(snap) {
			return snap.value ?? snap.base ?? void 0;
		}
		function useScopeValue(form) {
			const [snap, setSnap] = (0, react.useState)(() => getSnapshotValue(form));
			(0, react.useEffect)(() => {
				setSnap(getSnapshotValue(form));
				return subscribe(form, () => setSnap(getSnapshotValue(form)));
			}, [form]);
			return {
				snap,
		...
		function SettingsCard({ scope, loadModelCatalog, loadShelfInventory, t }) {
			...
			const save = (0, react.useCallback)(async () => {
				...
				try {
					...
					if (ops.length > 0 && !await scope.mutate(ops)) throw new Error("the Host refused the change; nothing was saved");
				} catch (err) {
			...
			if (snap.status === "unavailable") return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
				className: SettingsCard_module_css_default.status,
		...
		*/
		const fetchShelves = async () => {
			...
			if (inventory === void 0) throw new Error("the Host returned an unreadable shelf listing");
			return inventory;
		};
		/**
		...
		*/
		function parseShelfInventory(value) {
			if (!isRecord(value)) return void 0;
			return {
				shelves: Array.isArray(value.shelves) ? value.shelves.flatMap((shelf) => {
		...
		*/
		...
		const SETTINGS_TAB_SEAT = "settings.plugins.tab";
		/**
		...
		*/
		function installPluginCard(host, seats) {
			...
			const reconcile = () => {
				if (stopped || reconciling || official !== void 0) return;
				reconciling = true;
				try {
			...
			const stopWaiting = host.slots.inject(SETTINGS_TAB_SEAT, () => {
				if (stopped || official !== void 0 || settingsTab !== void 0) return () => {};
			...
			const stopWatching = host.on?.("slots/changed", () => {
				reconcile();
			});
			return () => {
				stopped = true;
		...
		*/
		...
		function apply(ctx) {
			...
			const loadShelfInventory = fetchShelves;
			...
			const unregister = ctx.effect(() => configForms.whileServed([NS], () => installPluginCard(ctx, {
				official: () => ctx.slots.register({
					name: BUNDLE_CONFIG_SEAT,
					key: BUNDLE_NAME,
					locale: NS
				}, renderCard),
				settingsTab: () => ctx.slots.register({
					name: SETTINGS_TAB_SEAT,
					id: NS,
					label: () => t("title"),
					locale: NS
				}, renderCard),
